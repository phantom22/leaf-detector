% models/segmentation/wb-360gabor-10_pct_noise_9f-lin-pixel-classifier.mat
...
function d = pixel_descriptors(im)
    ...
    lin_y = imgaussfilt(ycbcrim(:,:,1), 1.5);
    
    labim = rgb2lab(im);
    [sobel,sobel_dir] = mysobel(hsvim(:,:,3), 1/2);

    lap = mylaplacian(lin_y);
    ...
    gabormag = imgaborfilt(lin(:,:,3), gabor_filters);
